...
def test_ffmpeg_is_looked_for_rather_than_assumed(monkeypatch, tmp_path):
    ...
    from pixel_patrol_deepsea import analyse

    stated = tmp_path / "my-ffmpeg"
    stated.write_text("")
    monkeypatch.setenv("PIXEL_PATROL_FFMPEG", str(stated))
    analyse.ffmpeg.cache_clear()
    assert analyse.ffmpeg() == str(stated)

    monkeypatch.delenv("PIXEL_PATROL_FFMPEG")
    monkeypatch.setattr(analyse.shutil, "which", lambda _name: "/usr/bin/ffmpeg")
    analyse.ffmpeg.cache_clear()
    assert analyse.ffmpeg() == "/usr/bin/ffmpeg"
...
def test_no_ffmpeg_anywhere_says_how_to_get_one(monkeypatch):
    ...
    monkeypatch.setattr(analyse.shutil, "which", lambda _name: None)
